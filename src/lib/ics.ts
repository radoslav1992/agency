 ...
 *
 * Съществува, защото Cloudflare Email Routing доставя само до потвърдени
 * адреси в акаунта — тоест само до студиото. Посетителят не може да получи
 * имейл с потвърждение по този път, затова получава файл, който сам слага в
 * календара си. Той носи и връзката за отказ, така че часът не живее само в
 * един браузърен раздел.
 *
 * Ако някой ден се добави истински доставчик на поща, `notify.ts` е мястото,
 * а този файл остава — прикачен към писмото.
 */
