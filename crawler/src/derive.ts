...
function deriveDomain(db: DatabaseSync, runId: string, domain: string): MetricRow[] {
  ...
  if (rawBrowser) {
    ...
    // --- Оперативни сигнали (за писмата, не за доклада) ---
    const contact = deriveContactSignals(rawBrowser);
    out.push(measuredBool('has_contact_form', contact.has_contact_form, url));
    out.push(
      contact.has_contact_form
        ? measuredNum('contact_form_field_count', contact.contact_form_field_count, url)
        : { key: 'contact_form_field_count', status: 'not_applicable' },
    );
    out.push(measuredBool('contact_only_email', contact.contact_only_email, url));
    out.push(measuredBool('order_by_email_text', contact.order_by_email_text, url));
    out.push(measuredBool('facebook_as_primary_contact', contact.facebook_as_primary_contact, url));
    out.push(measuredBool('has_live_chat', contact.has_live_chat, url));

    const docs = deriveDocumentSignals(rawBrowser, url);
    out.push(measuredNum('downloadable_form_count', docs.downloadable_form_count, url));
    out.push(measuredText('downloadable_form_types', docs.downloadable_form_types.join(',')));
    out.push(measuredBool('price_list_is_pdf', docs.price_list_is_pdf, url));
    out.push(measuredBool('catalog_is_pdf', docs.catalog_is_pdf, url));
