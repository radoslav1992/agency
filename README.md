...
├── pages/
│   ├── index.astro         начална страница — разказът (виж по-долу)
│   ├── services.astro      услуги, процес, цени, пилот и условия
...
| Скриване на секцията с цени | `SHOW_PRICING` в `src/pages/services.astro` |
| Надписите по секциите, на двата езика | `src/i18n/ui.ts` |
| Нова публикация | нов `.md` файл в `src/content/blog/` |

## Началната страница е разказ, не каталог

Редът на секциите не е подреден по важност, а по това как читателят стига до
решение: закачка → напрежение → метод → каталог → безплатно доказателство →
построено доказателство → кой съм.

| # | Секция | Компонент |
| --- | --- | --- |
| 1 | Заглавието и обещанието | `Hero.astro` |
| 2 | „Екипът ти губи часове…“ | `ScrollStatement.astro` |
| 3 | Открий → Построй → Работи | `Journey.astro` |
| 4 | Указателят с готовите агенти | `AgentIndex.astro` |
| 5 | Безплатната проверка на сайтове | `AnalyzerStrip.astro` |
| 6 | Работещите продукти | `Proof.astro` |
| 7 | За мен, блогът, поканата | `About` · `BlogPreview` · `CtaBanner` |

Услугите, процесът, цените, пилотът и условията се преместиха на
`/services/`. Началната ги подсказва и води към тях; справочникът живее на
собствен адрес, за да може да се класира сам и да се праща по имейл.

### Движението е на CSS, не на библиотека

Секции 2, 3, 4 и 6 се анимират по скрола чрез `animation-timeline` — без
GSAP, без Lenis, без Three.js. Причината е, че сайтът продава скорост:
анимацията върви извън главната нишка и не тежи нито килобайт JavaScript.

Три правила при пипане на тези секции:

1. **Базовото състояние е крайното.** Всяка анимация стои зад
   `@supports (animation-timeline: view())`. Браузър без поддръжка показва
   завършената секция, а не празен екран.
2. **`prefers-reduced-motion` се уважава изрично.** Анимация по скрол няма
   продължителност, тоест обичайното `animation-duration: 0.01ms` не я спира.
   Затова декларациите са и в `@media (prefers-reduced-motion: no-preference)`.
3. **Внимавай къде слагаш `transform`.** Той прави от елемента containing
   block за абсолютно позиционираните му деца. В `AgentIndex` заради това
   анимацията виси на връзката вътре в реда, а не на самия ред — иначе
   рисунката каца върху списъка вместо върху сцената.
...
`src/data/testimonials.ts` съдържа отзивите. Всеки е от името на екипа зад проекта, с линк към живия сайт през `project: { name, url, result }`.

Отзивите за собствени продукти на студиото носят `own: true` и картата им показва „собствен продукт на студиото“. Това е разликата между отзив и реклама — без маркера читателят ги приема за препоръка от независим клиент. Отзивите за чужди проекти нямат маркер, защото са точно това: клиентски.
