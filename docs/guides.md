...
Ключът е в `src/data/flags.mjs`:

```js
const GUIDES = false;   // true връща наръчниците цели
```

Смяна на реда и сливане на PR. Сливането само пуска нов деплой в Cloudflare —
един ход, без табло.

Изключено изчезват: рафтът, страниците на наръчниците, връзката в менюто,
редовете в `sitemap.xml` и в `llms.txt`, страницата след плащането, свалянето,
записването за известие и webhook-ът. Адресите връщат 404, а не скрита
страница с работещ бутон „Купи“.

**Защо не е в таблото.** Превключвателят трябва да действа при СТРОЕЖ, не при
заявка: сайтът е предрендериран, връзката в менюто е записана в четиридесет
готови HTML файла, а страниците се сервират като статични ресурси, без
Worker-ът да се събужда. Променлива, четена при заявка, няма как да ги
промени. В тази сметка Cloudflare няма поле за променливи при строеж, затова
ключът стои в кода. `GUIDES_ENABLED` в средата на строежа продължава да има
предимство, ако някога се появи къде да се сложи.
...
`process.env` се отваря при заявка. Половин изключено е по-лошо от включено.

**Payment Link-ът в Stripe НЕ се изключва оттук** — той живее там, не тук.
Спреш ли секцията, деактивирай и него (Stripe → Payment links → линка →
Deactivate), иначе някой със стар адрес плаща за файл, който сайтът вече не
подава: пари, взети срещу нищо.
